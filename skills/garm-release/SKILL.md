...
(Week anchor: the Monday of the week GARM Init was run. Python is used instead of `date -d` because macOS BSD `date` doesn't support it. If `python3` is not on PATH — e.g. Git Bash on Windows — use `python` instead.)
...
2. Content source:
   ...
   - **Skipped Codex**: write CR from `.claude/skills/garm-review/cr-template.md` with body "Code review skipped — trivial change." Verdict: `APPROVED with observations`.
...
1. Read fully @docs/MIMIR-rules.md
2. Update @docs/MIMIR.md following the rules
3. Run `bash .claude/skills/garm-compact/count-tokens.sh docs/MIMIR.md` to check token count

**Warning: If MIMIR.md exceeds ~20,000 tokens**, warn the user:

> "MIMIR.md is at ~X tokens. Consider running `garm-compact` to reduce it before committing."
...
Keep the pre-existing (non-GARM) documentation aligned with the code:
