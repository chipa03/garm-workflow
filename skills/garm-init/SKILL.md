---
name: garm-init
description: Initialize GARM workflow in a new project (creates docs structure and generates MIMIR.md)
disable-model-invocation: true
argument-hint: "name of the project to initialize"
---

# GARM Initialization Mode

You are now in **initialization mode** for setting up the GARM workflow.

## What is GARM?

GARM is a structured development workflow with four phases:
...
GARM guards the gate; Mímir keeps the memory — hence `MIMIR.md`

---

## Your Task

Initialize the GARM workflow for the project: **$ARGUMENTS**
...
Files (`MIMIR.md`, `MIMIR-rules.md`, `changelog_table.md`, `TESTING.md`) will be created in later phases after codebase analysis.
...
## Phase 4: Generate MIMIR.md

Based on the project type, generate `docs/MIMIR.md` using the appropriate sections.
...
After generating MIMIR.md, **stop and request user review**.
...
2. **Sections included** - List the sections added to MIMIR.md
...
- **Question**: "Please review the generated MIMIR.md. How would you like to proceed?"
- **Options**:
  1. **"Approved"** — MIMIR.md looks good, proceed to Phase 6
...
**Do NOT proceed to Phase 6 until the user explicitly approves the MIMIR.md.**

---

## Phase 6: Update GARM Skills

After user validation, update the other GARM skill files based on the **actual codebase architecture** documented in MIMIR.md.
...
> - The patterns and conventions documented in the validated MIMIR.md (Phase 5)
...
Replace the `[ADAPT_TO_PROJECT]` markers in the Technical Considerations section with concerns **relevant to this specific codebase**. The examples below are starting points - adapt based on what MIMIR.md documents:
...
- **Pattern Usage**: Which existing patterns to follow (from MIMIR.md)
...
Replace the `[ADAPT_TO_PROJECT: Guidance Sections]` comment block with guidance that matches **the actual architectural patterns in MIMIR.md**.
...
_These are examples. Create guidance sections based on what's actually in MIMIR.md - the major patterns, layers, and component types specific to this project._

**C. Documentation Impact Candidates**

Replace the `[ADAPT_TO_PROJECT]` marker inside the plan template's **Documentation Impact** section with the project's actual living documentation — every non-GARM document that code changes can leave stale, one bullet per doc with when it's affected. Draw from what Phase 2 discovered:
...
`checklist.md` ships with generic sections (Functional Requirements, Code Quality, Architectural Compliance, Error Handling, Security, Performance). Replace the `[ADAPT_TO_PROJECT]` comment block with **project-specific checklist sections** based on what matters for this codebase as documented in MIMIR.md.
...
_Build from MIMIR.md — what patterns does this project use? What quality criteria matter? What are common pitfalls?_
...
### 6.5 Update `garm-test`

**A. Test Commands**

Replace the `[TEST_COMMAND_*]` placeholders with the **actual test commands** used in this project (from MIMIR.md or discovered during exploration):
...
Adapt based on **what's actually tested in this project** and what the MIMIR.md documents about testing strategy. Examples:
...
Now that MIMIR.md is validated, create the supporting documentation files adapted to the project.
...
| --------- | ---- | ------------------------------- |
| `X.Y.Z+1` | 1    | chore: initialize GARM workflow |
```

- **Version**: SemVer format in backticks (e.g., `1.0.0`, `0.2.1`)
- **Week**: Project week number. Week 1 = the week when GARM Init was run.
...
- **vX.Y.Z+1 (GARM Initialization - Week 1, DD-MM-YYYY)**:
  - **Setup**: Initialized GARM workflow with docs structure
  - **Documentation**: Generated MIMIR.md with [project type] architecture
  - **Files Added**: docs/MIMIR.md, docs/MIMIR-rules.md, docs/2-changelog/changelog_table.md, docs/4-unit-tests/TESTING.md
```
...
**Adapt based on the validated MIMIR.md** - use the actual test framework, commands, and conventions discovered during codebase exploration:
...
[From MIMIR: actual framework name and version]

## Running Tests

\`\`\`bash
[From MIMIR: actual test commands]
\`\`\`

## Test Organization

[From MIMIR: actual test file locations and patterns]
...
[From MIMIR: actual coverage thresholds if defined, or "Not defined" if none]
```

---

### 3. `docs/MIMIR-rules.md` - Architecture Maintenance Rules

**Adapt based on the validated MIMIR.md** - reference the actual sections and terminology used:

```markdown
# Architecture Documentation Rules

[MIMIR.md](MIMIR.md) documents the [Project Name] architecture. After each
task (new feature, refactor, bug fix), determine if MIMIR.md needs updating.
...
- [List actual section names from MIMIR.md that might need updates]
...
Review: [List actual relevant section names from MIMIR.md]

### Minor Feature / Enhancement

Update: [List actual relevant section names from MIMIR.md]
...
- [ ] MIMIR.md generated with appropriate sections (Phase 4)
...
- [ ] **GARM skills updated** (Phase 6):
  ...
  - [ ] `garm-test`: `[ADAPT_TO_PROJECT]` markers replaced with actual test structure/priorities
- [ ] changelog_table.md initialized with version+1 (Phase 7)
- [ ] TESTING.md created, adapted to actual test setup (Phase 7)
- [ ] MIMIR-rules.md created, referencing actual MIMIR sections (Phase 7)
...
- **User review is mandatory**: Never skip Phase 5. The user must validate the MIMIR.md before proceeding
