---
name: garm-compact
description: Compact MIMIR.md when it exceeds recommended size - smart compression without losing relevance
disable-model-invocation: true
---

# MIMIR Compaction Mode

You are now in **compaction mode** - intelligently reducing MIMIR.md size while preserving its value.

## Why Compact?

MIMIR.md should not exceed _~20k tokens_. A bloated MIMIR:
...
```bash
bash .claude/skills/garm-compact/count-tokens.sh docs/MIMIR.md
```

Then read the full MIMIR.md and evaluate:
...
- **Question**: "MIMIR.md is at ~[X] tokens (target: ~10,000-15,000). Main bloat sources: [list top 3-5]. Proceed with compaction?"
...
- **Question**: "MIMIR.md is at ~[X] tokens — within acceptable range. Would you still like to compact it further?"
...
- **Question**: "Compaction complete: ~[X] → ~[Y] tokens ([Z]% reduction). Changes: [brief summary]. How does the compacted MIMIR.md look?"
...
- Consider splitting into MIMIR.md (core) + MIMIR-detailed.md (deep dives)
...
- MIMIR.md feels sluggish to read
