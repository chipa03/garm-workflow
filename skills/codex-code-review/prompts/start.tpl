...
If `{{TARGET}}` resolves to a file under `docs/1-plans/`, treat it as the **implementation plan**: read it, evaluate the diff against it. If not a path (e.g. a free-form label), skip "Plan conformance" and review against `docs/MIMIR.md` patterns plus the stated intent in the additional-context block below.
...
1. `docs/MIMIR.md`
2. `.claude/skills/garm-review/checklist.md` — single source of truth for the review checklist, severity classification, and approval gate. Do NOT read `.claude/skills/garm-review/SKILL.md`.
