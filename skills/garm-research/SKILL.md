...
## Step 0: Read fully @docs/MIMIR.md
...
Create a lightweight research plan (not a full GARM plan):
...
- **Options**: "Elaborate on findings" (dive deeper into specific results), "Save as memo" (write findings to `docs/6-memo/` after all), "Plan implementation" (create a GARM plan based on these results), "Done" (no further action needed)
...
- **Options**: "Elaborate on findings" (dive deeper into specific results), "Plan implementation" (create a GARM plan based on these results), "Done" (no further action needed)
...
- Basis for future GARM planning
