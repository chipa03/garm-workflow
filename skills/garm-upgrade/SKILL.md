---
name: garm-upgrade
description: Upgrade GARM workflow skills to a newer version while preserving project customizations
disable-model-invocation: true
argument-hint: "[path to new-GARM folder]"
---

# GARM Upgrade Mode

You are now in **upgrade mode** — merging a newer version of the GARM workflow into this project's existing, customized GARM skills.

## The Problem

Each project's GARM skills have two interleaved layers:
...
The user must have copied the new generic GARM skills into a staging folder before running this skill. Default location: `.claude/skills/new-GARM/`

If `$ARGUMENTS` is provided, treat it as the path to the staging folder. Otherwise use `.claude/skills/new-GARM/`.
...
Confirm the staging folder exists and contains GARM skills:
...
> "No staging folder found at `<path>`. Copy the new GARM workflow's `skills/` folder there first, then re-run."
...
**Pure workflow skills** (no project customizations): `garm-compact`, `garm-hotfix`, `garm-research`, `garm-init`, `codex-implement`, `codex-plan-review`, `codex-code-review`
...
**Customized skills** (have project-specific content): `garm-plan`, `garm-implement`, `garm-release`, `garm-review`, `garm-test`

**Renamed skills** — if a newer version renames a skill folder, treat the installed folder and its staging counterpart as the same skill under its new name (merge into the new name, then delete the old folder).

**Other non-GARM skills** in staging (e.g. future additions): Treat as new, or as pure workflow if they already exist.
...
Skill                 | Status              | Action
--------------------- | ------------------- | ------
garm-plan             | Updated (customized) | Extract + merge
garm-implement        | Updated (customized) | Extract + merge
garm-release          | Updated (customized) | Extract + merge
garm-review           | Updated (customized) | Extract + merge
garm-test             | Unchanged            | Skip
garm-compact          | Unchanged            | Skip
garm-hotfix           | Updated (pure)       | Replace
garm-init             | Updated (pure)       | Replace
garm-research         | Unchanged            | Skip
codex-plan-review     | Updated (pure)       | Replace
codex-code-review     | Unchanged            | Skip
codex-implement       | Unchanged            | Skip
```
...
- `DOC_IMPACT_CANDIDATES` — the candidates list inside the plan template's `## Documentation Impact` section, if present (may not exist in older installs)
- `PLAN_PREREQUISITES` — any additional living-docs lines in the Prerequisites list beyond MIMIR.md, if present

**From garm-implement/SKILL.md:**
- `PROJECT_NAME` — (confirm matches garm-plan)
- `LINT_COMMAND` — if present (may not exist in older versions)
- `TYPECHECK_COMMAND` — if present
- `TEST_COMMAND` — if present

**From garm-release/SKILL.md:**
...
**From garm-review/SKILL.md (checklist.md if already split):**
...
### 3.1 Checklist Extraction (garm-review)

**Old structure**: Checklist inline in `garm-review/SKILL.md`
**New structure**: Checklist in separate `garm-review/checklist.md`, template in `garm-review/cr-template.md`
...
### 3.2 Codex Integration (garm-plan, garm-implement)

**Old structure**: No Codex review steps
**New structure**: garm-plan has Step 3 (Codex plan review), garm-implement has Codex Code Review section

These are pure workflow additions — no project-specific content to migrate. They will be applied from the new template. The only project-specific part is the test commands in garm-implement's Codex pre-step, which come from the extracted context.

### 3.3 Codex Skills (codex-plan-review, codex-code-review, codex-implement)

If not installed yet, these are entirely new — copy from staging directly. The review skills reference `garm-review/checklist.md` and `garm-review/cr-template.md`, which will be populated with project content. If already installed, replace as pure workflow (see the `_common.sh` exception in Phase 1.2) — including the prompt templates, so every path they reference matches the new structure.
...
6. Resolve the `[ADAPT_TO_PROJECT]` line in the Prerequisites list: use extracted `PLAN_PREREQUISITES` if present; otherwise list any living docs a plan must respect beyond MIMIR.md, or delete the line if there are none

#### garm-implement/SKILL.md

1. Start from the new template (staging)
2. Replace `[PROJECT_NAME]` with extracted `PROJECT_NAME`
3. Replace `[LINT_COMMAND]`, `[TYPECHECK_COMMAND]`, `[TEST_COMMAND]` in the Testing Gate with extracted commands
   - If the old version didn't have Codex review (no test commands extracted), check the old garm-test for test commands, or ask the user
4. Adapt the Integration impact check comment block to the project's integration/E2E tooling (from the old garm-test content if present)

#### garm-release/SKILL.md
...
#### garm-review/SKILL.md + checklist.md + cr-template.md
...
```bash
grep -rn '\[ADAPT_TO_PROJECT\|\[PROJECT_NAME\]\|\[VERSION_FILE\]\|\[WEEK_ANCHOR_DATE\]\|\[TEST_COMMAND\]\|\[LINT_COMMAND\]\|\[TYPECHECK_COMMAND\]\|\[TUTORIAL_STEP\]\|\[MAIN_BRANCH\]' .claude/skills/garm-*/
```
...
- `codex-code-review/prompts/start.tpl` and `resume.tpl` reference `.claude/skills/garm-review/checklist.md` — confirm it exists, and that no template still points at an outdated path
- `codex-code-review/prompts/synthesize.tpl` and `codex-code-review/SKILL.md` reference `.claude/skills/garm-review/cr-template.md` — confirm it exists
- `garm-plan` and `garm-implement` reference `codex-plan-review/scripts/start.sh` and `resume.sh`; `garm-implement` also references `codex-implement/scripts/start.sh` — confirm they exist
...
2. Report completion:
   > "GARM workflow upgraded. The staging folder has been removed. You can `git diff .claude/skills/` to review all changes before committing."
...
This is the most common upgrade path. The Codex skills are "New" — copy directly. The Codex integration in garm-plan and garm-implement comes from the new template and needs no project-specific content except test commands.
...
If the old version predates the Codex review pre-step and garm-test doesn't have extractable commands, ask the user:
