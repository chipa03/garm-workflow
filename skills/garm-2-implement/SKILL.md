...
1. @docs/MIMIR.md - Understand current system architecture
...
1. **Review the delta only**: `git status -s && git diff` — worktree vs index shows just this batch, since previous batches are staged (step 4). Check it against the plan, MIMIR.md patterns, and project conventions (DRY, KISS, comment discipline, error-handling and naming conventions from MIMIR.md).
...
If the change adds new logic, write its tests **now**, guided by the plan's **Test Impact** section and the project's testing guide (see `garm-test`). If no new logic was added, skip this step.
